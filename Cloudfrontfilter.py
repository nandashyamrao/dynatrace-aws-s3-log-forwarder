...
@metrics.log_metrics
    ...
    TARGET_BUCKET = "aws-waf-logs-sf-mgmt-prod-us-east-1"
...
def extract_bucket_name(message):
    """
    Attempts to extract the S3 bucket name from multiple common formats:
    - S3 Event notification (Records[0].s3.bucket.name)
    - CloudTrail style (Records[0].requestParameters.bucketName)
    - EventBridge event format (detail.bucket.name)
    """
    try:
        # Format 1: S3 put event or SNS-based S3 event
        if "Records" in message:
            record = message["Records"][0]
            if "s3" in record:
                return record["s3"]["bucket"]["name"]
            elif "requestParameters" in record:
                return record["requestParameters"].get("bucketName")

        # Format 2: EventBridge → SNS (used in cross-account log forwarding)
        if "detail" in message:
            return message["detail"].get("bucket", {}).get("name")

    except Exception as e:
        print(f"Error extracting bucket name: {e}")
