...
Changes in 7.5 (formatting safety fixes):
- Preserve original indentation/spacing for all DQL (no global re-indent pass)
- Respect comment lines starting with '#'
- Only minimal formatting:
  * Split pipe stages '|' onto their own lines (outside of strings)
  * Optionally soft-wrap very long concat(...) argument lists
- Variables section still rendered with a usage table

Reads all *.json in current folder and emits <stem>_TileQueries.md with title:
  "<DashboardJsonFileName> - Tile Query Details"
"""
...
def build_var_usage(tiles: List[Dict[str, Any]], var_map: Dict[str, str]):
    ...
    usage: Dict[str, List[Tuple[str,int]]] = {vn: [] for vn in var_map.keys()}
    for t in tiles:
        used_here = set()
...
def _split_pipes_preserve_indent(text: str) -> str:
    """Split on '|' outside strings, preserving all original spaces and comments."""
    # We operate on each original line independently to preserve leading spaces.
    lines = text.replace('\r\n', '\n').split('\n')
    result_lines = []
    for line in lines:
        if not line.strip():
        ...
        if line.lstrip().startswith('#'):
            # comment line: keep exactly as-is
            result_lines.append(line)
            continue
        # walk the line and split on pipes that are outside strings
        buf = ''
        out_parts = []
        in_s = False
        qch = ''
        ...
        while i < len(line):
            ...
            if in_s:
                ...
                if ch in ('"', "'"):
                    ...
                    buf = '|'  # keep pipe at start of next part
                else:
                    buf += ch
            i += 1
        out_parts.append(buf.rstrip())
        # Rebuild with newline before each subsequent pipe part; preserve leading spaces of original.
        leading = len(line) - len(line.lstrip(' '))
        indent = ' ' * leading
        if len(out_parts) == 1:
            result_lines.append(line)
        else:
            # first part on its own line
            first = out_parts[0]
            if first.strip():
                result_lines.append(indent + first.strip())
            for part in out_parts[1:]:
                if part.strip():
                    # ensure a space after pipe if not present
                    part_str = part if part.startswith('|') else '|' + part
                    if part_str.startswith('|') and not part_str.startswith('| '):
                        part_str = '| ' + part_str[1:]
                    result_lines.append(indent + part_str.strip())
    return '\n'.join(result_lines)

def _soft_wrap_concat_calls(text: str, max_len=MAX_LINE, inner_indent='      '):
    """Soft-wrap concat(...) arguments only when overlong; preserve everything else."""
    # Find concat( ... ) spans outside strings; for each, if its total length > max_len,
    # break arguments by commas at top level.
    s = text
    ...
    while True:
        ...
        for k,ch in enumerate(args_str):
            if in_s2:
                ...
                if ch == q2:
                    in_s2 = False
                elif ch == '\\' and k+1 < len(args_str):
                    buf += args_str[k+1]
                continue
            if ch in ('"', "'"):
                in_s2 = True; q2 = ch; buf += ch; continue
            if ch in '([{': lvl2 += 1
...
def format_dql_safe(q: str) -> str:
    # Preserve original indentation and comments; only split pipes, and wrap long concat calls.
...
def render_markdown(tiles: List[Dict[str, Any]], json_filename: str, var_map: Dict[str, str]) -> str:
    ...
    if var_map:
        ...
        for vn, vdql in var_map.items():
            ...
            for tname, qidx in usage.get(vn, []):
                by_tile[tname].append(qidx)
            if by_tile:
                md.append("| Tile | Query # |\n|---|---|\n")
                for tname in sorted(by_tile.keys(), key=str.lower):
                    qnums = ", ".join(str(n) for n in sorted(by_tile[tname]))
                    md.append(f"| [{tname}](#{anchor_for(tname)}) | {qnums} |\n")
                md.append("\n")
            if vdql:
                md.append("```sql\n")
