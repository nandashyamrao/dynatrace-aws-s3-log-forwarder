...
# -*- coding: utf-8 -*-
"""
Dynatrace Dashboard -> Markdown extractor (v7.6)

What's new in 7.6
- Variables usage now records the **exact query indices per tile** reliably.
- Variables table shows unique, sorted query numbers for each tile.
- Keeps all 7.5 fixes: safe formatting (preserve indentation/comments), split pipes, wrap long concat().

Outputs <stem>_TileQueries.md for each *.json in current folder, titled:
...
def build_var_usage(tiles: List[Dict[str, Any]], var_map: Dict[str, str]):
    """
    Return usage dict: var -> list of (tile_name, query_index).
    Annotate tiles with vars_used (unique, sorted).
    """
...
def render_markdown(tiles: List[Dict[str, Any]], json_filename: str, var_map: Dict[str, str]) -> str:
    ...
    if var_map:
        ...
        for vn, vdql in var_map.items():
            md.append(f"### `{vn}`\n\n")
            # Build usage table with unique, sorted query indices per tile
            by_tile = defaultdict(list)
            for tname, qidx in usage.get(vn, []):
                if qidx not in by_tile[tname]:
