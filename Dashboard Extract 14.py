...
What's new in 7.7
- Variables section now shows a **compact 4-column table** of tile links (no query numbers)
  to reduce vertical scrolling.
- Keeps v7.6 behavior: safe DQL formatting (preserve indentation/comments), split pipes,
  wrap long concat(); scans all *.json and writes <stem>_TileQueries.md with the title
...
def render_markdown(tiles: List[Dict[str, Any]], json_filename: str, var_map: Dict[str, str]) -> str:
    ...
    if var_map:
        ...
        for vn, vdql in var_map.items():
            ...
            tiles_using = sorted({tname for (tname, _q) in usage.get(vn, [])}, key=str.lower)
            if tiles_using:
