    ...
    for index, message in enumerate(event.get("Records", [])):
        try:
            ...
            if message.get("eventSource") == "aws:sqs":
                logging.info("SQS message received. Proceeding to process as SQS.")
            ...
            # Handle raw SNS
            elif message.get("eventSource") == "aws:sns":
                logging.info("SNS message received. Processing as raw message delivery.")
                try:
                    ...
                    logger.warning(f"Failed to process raw SNS message: {e}")

            else:
                logger.warning("Skipping non-SQS and non-SNS event")
