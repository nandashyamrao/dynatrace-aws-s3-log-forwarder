...
Changes in 7.4:
- Variables section now renders **usage as a Markdown table** (Tile + Query #) with links
- Still reads all *.json in the current folder
- Title: "<DashboardJsonFileName> - Tile Query Details"
- Simple DQL formatting: pipe-per-line + soft wrap top-level commas
"""
...
def format_dql_simple(q: str) -> str:
    q = q.replace('\r\n', '\n').strip()
    s = _split_on_pipes(q)
    lines = []
    for ln in s.splitlines():
        if ln.startswith('| '):
            lines.append('| ' + _soft_wrap_top_level_commas(ln[2:].strip(), MAX_LINE, indent='    '))
        else:
...
def render_markdown(tiles: List[Dict[str, Any]], json_filename: str, var_map: Dict[str, str]) -> str:
    ...
    if var_map:
        ...
        for vn, vdql in var_map.items():
            md.append(f"### `{vn}`\n\n")
            # Group usage by tile and collect query indices
            ...
            if vdql:
                md.append("```sql\n")
                md.append(format_dql_simple(vdql))
                md.append("\n```\n\n")
            else:
