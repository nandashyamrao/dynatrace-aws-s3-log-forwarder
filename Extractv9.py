#!/usr/bin/env python3
"""
Dynatrace Dashboard -> Markdown extractor (v7.4)
...
- Minimal DQL formatting:
  ...
  * If a line is very long, soft-wrap at top-level commas with a simple indent
  * No aggressive re-grouping/keyword rewriting

Usage:
    python3 extract_dt_dashboard_to_markdown_v7_4.py
"""
...
# ---- Keys we commonly see in Dynatrace dashboards ----
QUERY_KEYS = {
    "query", "dql", "ql",
...
# ------------ Minimal DQL formatter ------------
MAX_LINE = 120

def _split_on_pipes(text: str) -> str:
    """Split on | outside of strings; begin each stage on a new line."""
...
def _soft_wrap_top_level_commas(line: str, max_len=MAX_LINE, indent='  ') -> str:
    """If line is long, wrap at top-level commas with indent. Leaves content unchanged."""
    if len(line) <= max_len:
        return line
    ...
    if len(args) <= 1:
        return line  # nothing to wrap nicely
    return (',\n' + indent).join(args)

def format_dql_light(q: str) -> str:
    """Minimal, non-invasive formatting."""
    q = q.replace('\r\n', '\n').strip()
    s = _split_on_pipes(q)
    # soft-wrap long lines only
    lines = []
...
def render_markdown(tiles: List[Dict[str, Any]], file_name: str) -> str:
...
def main():
    ...
    for p in json_files:
        try:
            ...
            print(f"Skipping {p.name}: failed to parse JSON ({e})", file=sys.stderr)
            continue
        ...
        out_path = p.with_name(f"{p.stem}_TileQueries.md")
        md = render_markdown(tiles, file_name=p.name)
