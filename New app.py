...
logger.info(
    ...
    current_log_processing_rules_version,
    os.environ.get('LOG_FORWARDER_CONFIGURATION_LOCATION')
)
    ...
    if os.environ['LOG_FORWARDER_CONFIGURATION_LOCATION'] == "aws-appconfig":
        ...
        try:
            ...
            if rules_configuration_profile['Configuration-Version'] != glob[f"current_log_{rules_type}_rules_version"]:
                logger.info(
                    "🔄 New log-%s-rules version found: %s. Reloading...",
                    rules_type,
                    rules_configuration_profile['Configuration-Version']
                )
...
def lambda_handler(event, context):
    ...
    logger.debug("🔍 Raw Event: %s", json.dumps(event, indent=2))
    ...
    for index, message in enumerate(event_records):
        ...
        try:
            ...
            logger.info(
                "✅ Matched forwarding rule '%s' for s3://%s/%s",
            ...
            if matched_log_processing_rule is None:
                logger.warning(
                    "⚠️ No processing rule matched for source %s and key %s",
