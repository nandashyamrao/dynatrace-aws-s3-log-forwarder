...
@metrics.log_metrics
# 👋 This is the entry point for your AWS Lambda function
# When a new message arrives (via SQS), this function runs
def lambda_handler(event, context):
    """
    Main Lambda handler function to process messages coming via SQS.
    Handles two types of wrapped content:
    1. From SNS → SQS
    2. From EventBridge → SQS
    """
    ...
    # Prepare list to track batch failures
    batchItemFailures = []

    # Loop through incoming records from SQS
    for i, record in enumerate(event.get('Records', [])):
        try:
            # Decode the outer SQS message
            body = json.loads(record['body'])
            logger.debug("Decoded body: %s", body)

            # Case 1: Message is wrapped in SNS
            if 'TopicArn' in body:
                logger.info("Message detected as coming from SNS → SQS")
                sns_message = json.loads(body['Message'])  # unwrap one more layer
                inner_event = sns_message  # Final message

            # Case 2: Message is from EventBridge → SQS
            elif 'detail-type' in body and 'source' in body:
                logger.info("Message detected as EventBridge → SQS")
                inner_event = body  # Already at correct layer

            else:
                logger.warning("Unknown message format: skipping record")
                continue

            # Extract bucket and key if they exist
            try:
            try:
                formatted_msg = json.dumps(inner_event, indent=2)
                logger.info("Full message content:\n%s", formatted_msg)
            try:

                detail = inner_event.get("detail", {}).get("requestParameters", {})
                bucket_name = detail.get("bucketName")
                object_key = detail.get("key")
                if isinstance(object_key, dict):
                object_key = object_key.get("key")

                if not bucket_name or not object_key:
                    logger.warning("Missing bucket or object key in message")
                    logger.warning("bucket_name: %s, object_key: %s", bucket_name, object_key)
                    continue
                    logger.warning("Missing bucket or object key in message")
                    logger.warning("bucket_name: %s, object_key: %s", bucket_name, object_key)
                    continue
            ...
            logger.info(f"Processing S3 log file: s3://{bucket_name}/{object_key}")

            # Call downstream processing function
                process_s3_event(bucket_name, object_key, inner_event, context)

            except Exception as e:
                import traceback
                logger.error("Error extracting bucket/key: %s", str(e))
            ...
            batchItemFailures.append({"itemIdentifier": record.get("messageId")})
...
# 📦 We're checking if the event has 'Records' - these are the incoming messages
    ...
    for index, message in enumerate(event_records):
        ...
        try:
# 🔍 'body' contains the actual message we want to process
# But sometimes it's JSON-encoded *twice* (especially with SNS)
# So we need to decode (parse) it properly
