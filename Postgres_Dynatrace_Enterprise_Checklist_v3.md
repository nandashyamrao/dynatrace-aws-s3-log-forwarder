...
- [ ] **Decide Visibility Level**  
  ...
  - **Change Scope:**  
    ...
    - Config change (`shared_preload_libraries`) may be needed.

  ### 🔹 Level C — + Execution Plans (Deep Diagnostics)
  - **What You Get:**  
    - Execution plan for top queries (EXPLAIN output in JSON)  
    - Visibility into index usage and query execution path  
    - Estimated vs actual rows and costs  
  ...
  - **Change Scope:**  
    ...
    - Security review to confirm no arbitrary SQL execution risk
...
### 📋 Dynatrace Documentation Checklist (Verify in Environment)

| Area | What to Verify | Why |
|------|----------------|-----|
| **ActiveGate** | AG version ≥ 1.269, outbound HTTPS to `*.live.dynatrace.com`, proxy configured | Required for Extension 2.0 + TLS |
| **Certificates** | DB TLS enabled, CA trusted by AG | Ensures secure connection and avoids SSL errors |
| **Postgres Version** | PostgreSQL ≥ 11 | Lower versions not officially supported |
| **pg_stat_statements** | Installed & loaded via `shared_preload_libraries` | Needed for Top Queries (Level B) |
| **Dynatrace Credential Vault** | Used for storing DB user credentials | No plaintext passwords in config |
| **Feature Sets** | Decide which metrics feature sets to enable | Controls data volume + DDU consumption |
| **Execution Plan Function** | Schema/function created by DBA, EXECUTE granted only to `dynatrace` | Required for Level C (Deep Diagnostics) |
| **Audit & Logging** | Log DB connections for `dynatrace` user | Compliance and troubleshooting |
| **Scale Considerations** | Number of endpoints per config (< 20k) | Avoid hitting Dynatrace limits |
