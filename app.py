...
# Read the allowed bucket name from environment variable
ALLOWED_BUCKET = os.environ.get("ALLOWED_CLOUDFRONT_BUCKET")
    ...
    for index, message in enumerate(event_records):
        ...
        try:
            if 'detail' in payload:
                ...
                logger.warning("Unsupported event format, skipping.")
                continue
        except Exception:
        ...
        try:
            ...
            if matched_log_processing_rule is None:
                logger.warning("No processing rule matched for source %s and key %s",
            ...
            if bucket_name == ALLOWED_BUCKET:
                logger.info("✅ Allowed bucket %s — starting log processing", bucket_name)
                ...
                logger.warning({
                    "message": "⛔ Skipping log file from disallowed bucket",
                    "received_bucket": bucket_name,
                    "allowed_bucket": ALLOWED_BUCKET,
                    "key": key_name
                })
                metrics.add_metric(name='SkippedBucketsDueToFilter', unit=MetricUnit.Count, value=1)
