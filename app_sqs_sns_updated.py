...
def lambda_handler(event, context):
    ...
    for index, message in enumerate(event_records):
        try:
            # Check if the message is from SQS
            if message.get("eventSource") == "aws:sqs":
                logging.info("SQS message received. Proceeding to process as an SQS message for CloudTrail events.")
                sqs_message = json.loads(message["body"])
                bucket_name = sqs_message.get("Records", [{}])[0].get("s3", {}).get("bucket", {}).get("name")
                object_key = sqs_message.get("Records", [{}])[0].get("s3", {}).get("object", {}).get("key")

                if bucket_name and object_key:
                    s3_event = {"bucket_name": bucket_name, "object_key": object_key}
                    process_s3_event(s3_event, context)

            # Check if the message is from SNS
            elif "sns" in message:
                logging.info("SNS message received. Proceeding to process as an SNS message for CloudTrail events.")
                sns_message = message["sns"]["Message"]
                sns_subject = message["sns"].get("Subject", "")

                if "Amazon S3 Notification" in sns_subject or "Amazon S3 Notification" in sns_message:
                    logging.info("Processing SNS message related to an S3 event.")
                    try:
                        sns_event = json.loads(sns_message)
                    except json.JSONDecodeError as exception:
                        logging.warning(f"Failed to parse SNS message: {exception}")
                        continue  # skip this message

                    bucket_name = sns_event.get("Records", [{}])[0].get("s3", {}).get("bucket", {}).get("name")
                    object_key = sns_event.get("Records", [{}])[0].get("s3", {}).get("object", {}).get("key")

                    if bucket_name and object_key:
                        s3_event = {"bucket_name": bucket_name, "object_key": object_key}
                        process_s3_event(s3_event, context)
                else:
                    logger.warning("SNS message not related to S3. Skipping.")
            else:
                logger.warning("Skipping non-SNS and non-SQS event.")
        except Exception as e:
            logger.exception("Error processing record", exc_info=True)
