...
This guide explains two approaches to providing configuration for your Lambda function:

- **Option A:** Use AWS AppConfig (recommended for production and dynamic config updates)
- **Option B:** Store configuration files locally in the Lambda deployment package (simple and AppConfig-free, suitable for testing/dev)

It covers naming conventions, required resources, Lambda environment variables, IAM permissions, and how to automate setup with Terraform. It also explains how the build and deployment flow works for both methods, and how `app.py` supports local config loading.

---

## Option A: Using AWS AppConfig (Recommended)
...
### 3. **Lambda Environment Variables (Essentials with AppConfig)**
...
### 5. **Terraform Code Example for AppConfig Setup and Policy**
...
### 6. **Build & Deployment Flow Explanation (AppConfig)**

1. **Terraform Build:**  
   - Provisions all AppConfig resources: Application, Environment, Configuration Profiles, Hosted Versions, Deployment Strategy, and Deployments.
   - Creates an IAM policy (using resource IDs in ARNs) granting Lambda permission to retrieve configuration.
...
## Option B: Local Configuration Files (No AppConfig Required)

If you prefer not to use AWS AppConfig, you can store your configuration files locally within the Lambda deployment package.  
This is ideal for development, testing, or simple static config needs.

### **How to Use Local Configuration**

1. **Place Config Files in `config/` Folder:**  
   - Create a `config/` directory at the root of your Lambda source code.
   - Place your JSON configuration files inside, e.g.:
     - `config/log-forwarding-rules.json`
     - `config/log-processing-rules.json`

2. **Set Environment Variable to Use Local Files:**  
   - Set the following environment variable for your Lambda function:
     ```
     CONFIG_SOURCE=local
     ```
   - (You may also set or ignore any AppConfig-related variables; they will not be used.)

3. **How `app.py` Supports This Option:**  
   - The Lambda handler in `app.py` checks the value of `CONFIG_SOURCE`.
   - If `CONFIG_SOURCE=local`, the code loads configuration directly from the local `config/` folder, bypassing AppConfig entirely.
   - If `CONFIG_SOURCE` is not set (or set to another value), AppConfig will be used as per Option A.

4. **No AppConfig or IAM Policy Required:**  
   - You do **not** need to create any AppConfig resources.
   - You do **not** need to attach any AppConfig IAM permissions to your Lambda role.
   - Your Lambda function will always use the configuration bundled at deployment time.
...
├── app.py
└── config/
    ├── log-forwarding-rules.json
    └── log-processing-rules.json
```

#### Example: Lambda Environment Variable
...
### **Option B Build & Deployment Flow**

1. **Prepare Local Config Files:**  
   - Edit your JSON files in `config/` as needed.

2. **Deploy Lambda:**  
   - Deploy your Lambda with the `config/` folder included in the deployment package.
   - Set `CONFIG_SOURCE=local` in the Lambda environment variables.

3. **Execution:**  
   - On invocation, `app.py` detects `CONFIG_SOURCE=local` and loads the config from disk.

4. **To Update Config:**  
   - Update the files in `config/` and redeploy the Lambda function.

---

## **Comparison Table**

| Feature                     | Option A: AppConfig          | Option B: Local Files      |
|-----------------------------|------------------------------|---------------------------|
| Dynamic Config Updates      | Yes (no redeploy needed)     | No (requires redeploy)    |
| IAM Permissions Needed      | Yes (AppConfig access)       | No                        |
| AWS Resources Needed        | Yes (AppConfig setup)        | No                        |
| Runtime Overhead            | Slightly higher (API calls)  | None                      |
| Recommended For             | Production, dynamic configs  | Dev, static configs, test |

---

## 7. **Summary Checklist**

- [ ] Chosen config approach (`AppConfig` or `local files`) and set environment variables accordingly.
- [ ] If using AppConfig: Application, Environment, Profiles, Versions, Deployment Strategy created (manually or with Terraform).
- [ ] If using AppConfig: Lambda execution role has required AppConfig permissions (using resource IDs).
- [ ] If using local files: `config/` folder included in Lambda package and `CONFIG_SOURCE=local` set.
