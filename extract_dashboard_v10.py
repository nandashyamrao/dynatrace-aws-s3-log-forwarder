...
- For each dashboard JSON, extracts tiles with queries, descriptions, visualization info
- Adds a Variables section with each variable's DQL (supports 'input' key) + links to tiles using it
- Generates a Markdown file named <json_stem>_TileQueries.md
- Titles each Markdown: "<DashboardJsonFileName> - Tile Query Details"
- Keeps DQL formatting simple: pipes on separate lines, correct indentation, no over-aggressive breaking

Usage:
    python3 "Dashboard Extract 7.3.py"
"""
...
# ----------------- DQL simple pretty printer -----------------

def format_dql_query(q: str) -> str:
    """Simple formatting: put each pipe stage on its own line, indent wrapped lines."""
    q = q.replace("\r\n", "\n").strip()
    # split by pipe outside strings
    out, buf, in_s, qch = [], "", False, ""
...
def main():
    here = pathlib.Path(".")
    for json_file in here.glob("*.json"):
        ...
        out_path = here / f"{json_file.stem}_TileQueries.md"
