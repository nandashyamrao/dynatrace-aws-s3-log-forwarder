...
# Dynatrace Dashboard -> Markdown Extractor (v4: Visualization Column)

Adds a **Visualization** column to the single overview table (TOC + Summary).  
It attempts to read visualization info from common places in Dynatrace dashboard JSON:
- `visualConfig.type`
- `visualization`
- `viz`
- `chartType`
...
If nothing is found, the value is set to `—`.

---

## Usage
```bash
python3 extract_dt_dashboard_to_markdown_v4.py dashboard.json
...
#!/usr/bin/env python3
"""
Dynatrace Dashboard -> Markdown extractor (v4)
- Single table that acts as both Table of Contents and Summary
- NEW: Visualization column (reads from visualConfig.type / visualization / viz / chartType / graphType)
...
def detect_visualization(node: Dict[str, Any]) -> str:
    ...
    # Sometimes visualization is embedded inside options/configs
...
def render_markdown(tiles, title: str) -> str:
    ...
    # Linked Overview table with Visualization column
    md.append("## Overview\n")
...
def main():
    if len(sys.argv) < 2:
        print("Usage: extract_dt_dashboard_to_markdown_v4.py <dashboard.json> [output.md]", file=sys.stderr)
        sys.exit(1)
