#!/usr/bin/env python3
"""
Dynatrace Dashboard -> Markdown extractor (v5.1)
- Robust variable discovery + DQL extraction
- Variables section with per-variable DQL + tile usage
- Overview table includes Vars Used
- Keeps visualization column + collapsible query details
"""
...
def parse_variables(data: Dict[str, Any]) -> Dict[str, str]:
    ...
    Scans common places AND falls back to a generic scan for objects that look like variable definitions.
    """
    var_map: Dict[str, str] = {}

    def add_var(name: str, dql: str):
        if not name or not dql:
            return
    ...
    for path in [
        ["variables"],
        ["dashboardMetadata", "variables"],
        ["inputs"],  # some exports
    ]:
        node = get_nested(data, path) if isinstance(path, list) else data.get(path)
        if isinstance(node, list):
            for v in node:
                ...
                for key in ("query", "dql", "definition", "data"):
                    val = v.get(key)
    ...
    # 2) Generic scan for variable-like objects anywhere in JSON
    def walk_for_vars(n: Json):
            ...
            for key in ("query", "dql", "definition", "data"):
                ...
                if isinstance(val, str) and looks_like_dql(val):
                    ...
                    for kk in ("query", "dql", "data", "string"):
                        ...
                        if isinstance(vv, str) and looks_like_dql(vv):
                            possible.append(vv)
            if name and possible:
                add_var(str(name), possible[0])
            for v in n.values():
                walk_for_vars(v)
...
def main():
    if len(sys.argv) < 2:
        print("Usage: extract_dt_dashboard_to_markdown_v5_1.py <dashboard.json> [output.md]", file=sys.stderr)
        sys.exit(1)
