#!/usr/bin/env python3
"""
Dynatrace Dashboard -> Markdown extractor (v6)
- Variables section with each variable's DQL (or "definition not found")
- Per-variable list of tiles (links) where it's used
- Overview table includes "Vars Used" column
- Visualization column + collapsible query details
- Extremely robust variable discovery:
  * variables, dashboardMetadata.variables, inputs
  * definition/query/dql/data/string inside nested objects
  * falls back to scanning entire JSON for variable-like objects
"""
...
VAR_VALUE_KEYS = ("query", "dql", "definition", "data", "string", "value", "expression")

def extract_dql_from_obj(obj: Dict[str, Any]) -> str:
    # Search typical keys first
...
def parse_variables(data: Dict[str, Any]) -> Dict[str, str]:
    """Return {var_name -> dql_string}. Falls back to deep scan if needed."""
    ...
    def add_var(name: str, dql: str):
        ...
        if dql:
            ...
            var_map[name] = ""  # placeholder when definition missing
    ...
    # Deep scan: any object that looks like variable definition
    def walk(n: Json):
        if isinstance(n, dict):
            name = n.get("name") or n.get("key") or n.get("id")
            # Heuristic: if name looks like a variable or resides in a "variable"ish object,
            # try to pull DQL from it
...
def render_markdown(tiles, title: str, var_map: Dict[str, str]) -> str:
    ...
    # Variables section (always show section if any variables were detected)
    if var_map:
        md.append("## Variables\n")
...
def main():
    if len(sys.argv) < 2:
        print("Usage: extract_dt_dashboard_to_markdown_v6.py <dashboard.json> [output.md]", file=sys.stderr)
        sys.exit(1)
